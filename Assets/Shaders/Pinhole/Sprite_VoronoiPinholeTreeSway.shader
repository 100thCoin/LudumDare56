Shader "Custom/TreePinholes"
{
	Properties
	{
		...
		_Tex_Trunk ("Tree Base", 2D) = "white" {}
		_Tex_Leaf_F ("Leaves Front", 2D) = "white" {}
		_Tex_Leaf_B ("Leaves Back", 2D) = "white" {}
		_Tex_Leaf_Sway ("Leaves Distort", 2D) = "white" {}
		_LeafSwayMagnitude ("Leaf Sway Magnitude", Range(0, 5)) = 1
	...
	SubShader
	{
		...
		sampler2D _Tex_Trunk;
		sampler2D _Tex_Leaf_F;
		sampler2D _Tex_Leaf_B;
		sampler2D _Tex_Leaf_Sway;
		...
		float Pinhole(v2f i)
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float4 trunk = tex2D(_Tex_Trunk, i.uv);
				float4 leaf_LUT = tex2D(_Tex_Leaf_Sway, i.uv);
				float4 leaf_f = tex2D(_Tex_Leaf_F, i.uv + float2((sin(_Time.x*10 + leaf_LUT.g*1.5 + i.wpos.x*0.1)*(leaf_LUT.r*_LeafSwayMagnitude)*0.03 - 0.015f*leaf_LUT.r*_LeafSwayMagnitude),0));
				float4 leaf_b = tex2D(_Tex_Leaf_B, i.uv + float2((sin(_Time.x*10 + leaf_LUT.g*1.5 + i.wpos.x*0.1)*(leaf_LUT.r*_LeafSwayMagnitude)*0.03 - 0.015f*leaf_LUT.r*_LeafSwayMagnitude),0));

				float4 LayerCompile = (leaf_b * saturate(1-trunk.a) + trunk*trunk.a)*saturate(1-leaf_f.a)+leaf_f*leaf_f.a;
